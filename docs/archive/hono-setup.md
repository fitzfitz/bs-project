...
**Version:** 2.1.0  
**Status:** Approved — Updated for API versioning (Sprint 6)  
...
```text
/src
├── /features           # Domain-specific modules (30 modules)
│   └── /<feature>
...
// ... 28 more feature imports (incl. notifications)
...
// Mount feature routes (30 modules)
...
// V1 version header
apiApp.use("*", async (c, next) => { await next(); c.header("X-API-Version", "v1"); });
...
v2App.use("*", async (c, next) => { await next(); c.header("X-API-Version", "v2"); });
v2App.doc("/openapi.json", { openapi: "3.1.0", info: { title: "TMNG SaaS API v2", version: "2.0.0" } });
...
> **API Versioning:** The `/api/v2` mount point is scaffolded and ready for routes that introduce breaking changes. All responses include an `X-API-Version` header (`v1` or `v2`). Feature routes are registered on v1 by default; when a breaking change is needed, create a v2-specific handler and mount it on `v2App`.
