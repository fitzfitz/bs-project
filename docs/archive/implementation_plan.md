# TMNG SaaS Platform — Architecture & Implementation Plan

> **Status:** All 7 phases and Sprints 1–9 complete as of March 2026. **Sprint 9:** i18n (react-i18next, en/id), advanced reporting (PDF, SMTP, scheduled reports, saved templates), customer self-service (optional prepayment, cancellation policies/penalties, waitlist). **Post-Sprint 9 gap fixes:** admin waitlist page, client waitlist tab, PlatformConfig seeding, SMTP env vars. **822** Vitest tests (545 API + 167 admin + 110 client). See [audit_report.md](audit_report.md) for current feature status and [next_phase_plan.md](next_phase_plan.md) for full sprint history.

This document outlines the technical requirements, architecture, and implementation strategy for the TMNG SaaS Platform (originally "The Barber Project") based on the provided goals and features.
...
- **Payment Model:** Pay at checkout **or** optional online prepayment (Sprint 9) — org-configurable via `PREPAYMENT_ENABLED`, `DEPOSIT_PERCENTAGE`. When disabled, behavior matches pay-at-checkout-only.
- **Payment Gateway:** Xendit (tentative, may change) — abstracted behind a payment adapter interface for easy swapping (POS charges, saved cards, **optional booking prepayment**).
- **Booking Policy:** 10-minute grace period → auto-release → late arrivals treated as walk-ins. **Sprint 9:** Configurable cancellation policy (`CANCELLATION_POLICY_HOURS`, `CANCELLATION_PENALTY_PERCENTAGE`); **penalties apply only to prepaid bookings**; refunds tracked on `QueueEntry.refundAmount`.
- **Waitlist (Sprint 9):** Optional per org (`WAITLIST_ENABLED`, `WAITLIST_MAX_PER_SLOT`); `WaitlistEntry` lifecycle with expiry cron (every 5 minutes).
- **Reporting email (Sprint 9):** Transactional SMTP via `nodemailer` (`SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS`, `SMTP_FROM`) for scheduled report delivery; PDF generation via `pdfkit`.
- **i18n (Sprint 9):** Admin and client use `react-i18next` + `i18next` + `i18next-browser-languagedetector`; locales `en`, `id`; namespace JSON files — frontend-only, no API translation layer.
- **WhatsApp Notifications:** ✅ Implemented (Sprint 7) — Twilio WhatsApp Business API adapter in `NotificationService`, admin channel config, client preference toggle.
- **SMS Notifications:** ✅ Implemented (Sprint 8) — Twilio SMS adapter in `NotificationService`. Reuses same Twilio account, separate `TWILIO_SMS_FROM` number.
...
- **i18n (Sprint 9):** `react-i18next`, `i18next`, `i18next-browser-languagedetector`
- **Apps:**
  - **Client PWA** (`@tmng/barber-client`) — customer-facing booking & loyalty app (mobile-first)
  - **Admin Dashboard** (`@tmng/barber-admin`) — branch operations, POS, staff management, super admin analytics & configuration (desktop-first)
...
- **Sprint 9 libs:** `pdfkit` + `@types/pdfkit` (PDF reports), `nodemailer` + `@types/nodemailer` (SMTP for scheduled reports)
...
- 1.1.2. OAuth Login (Google)
- ~~1.1.3. Phone Number Verification (OTP)~~ — DESCOPED
...
- 1.3.6. Reschedule & Cancellation (configurable policy; penalties **only** when prepayment applies)
...
#### 1.6. Notifications (V1: Push + In-App Inbox)

- 1.6.1. ✅ Appointment Reminders (push — 30 min before via cron + in-app inbox record)
- 1.6.2. ✅ Booking Status Updates (confirmed, CALLED, COMPLETED — push + in-app inbox)
- 1.6.3. Promotions & Loyalty Milestone Alerts
- 1.6.4. ✅ Re-engagement Nudges (retention triggers via OneSignal push)
- 1.6.5. ✅ WhatsApp integration via Twilio adapter (Sprint 7). ✅ SMS via Twilio (Sprint 8).
- 1.6.6. **Tech Stack:** OneSignal for Web Push Notifications. In-app `Notification` model for persistent inbox.
- 1.6.7. ✅ In-App Notification Inbox — `GET /notifications`, `GET /notifications/unread-count`, `PATCH /notifications/:id/read`, `POST /notifications/mark-all-read`
- 1.6.8. ✅ Client bell icon with unread count badge + `/notifications` page
...
- **V1 Providers:** OneSignal for Web Push Notifications. In-app `Notification` model for persistent inbox.
- **Push triggers:** Booking confirmed, CALLED, COMPLETED, appointment reminders (30-min cron), retention nudges, campaigns
- **In-app inbox:** 4 API endpoints (list paginated, unread count, mark read, mark all read). Client bell icon + `/notifications` page.
- **Current Providers:** OneSignal (push), Twilio (WhatsApp + SMS). All gracefully degrade when env vars are absent.
- **Channel Config:** Admin toggles (push/WhatsApp/SMS) per notification type. User opt-out preferences per channel.
...
- API versioning for future-proofing — `/api/v2` mount point scaffolded (Sprint 6), `X-API-Version` header on all responses
...
- **Frontend:** Multi-stage Docker images (Node build + Nginx serve with SPA fallback), deployed to GHCR. Dockerfiles in `apps/client/Dockerfile` and `apps/admin/Dockerfile`.
...
- **Environments:** dev / staging / production with Zod-validated env vars (incl. **SMTP_*** for scheduled reports — Sprint 9)
- **DB Backups:** Automated daily PostgreSQL backups with point-in-time recovery

#### 5.8. Monitoring & Logging

- ~~Error tracking (Sentry for Node.js)~~ — DESCOPED (replaced by pino structured logging)
- **Structured logging:** pino (JSON in production, pino-pretty in development). All `console.log/error` replaced.
- **Request correlation:** `X-Request-Id` header (read from client or auto-generated UUID) propagated through all log lines.
- **Enhanced health endpoint:** `GET /api/health` returns uptime, memory stats (rss/heapUsed/heapTotal), DB pool stats (total/idle/waiting), and API version.
- **DB pool tuning:** Configurable `DB_POOL_MAX` env var (default 10), explicit warmup query on pool creation.
...
### Phase 2: Branch Operations ✅ Complete
...
| ~~Error Monitoring~~  | ~~Sentry for Node.js integration~~ — DESCOPED                              |


---

### Phase 3: Client Application ✅ Complete
...
### Phase 4: Financial & Workforce ✅ Complete
...
| What               | Features Covered                                                         |
| ------------------ | ------------------------------------------------------------------------ |
| POS                | Checkout (Services + Products tabs), CASH/QRIS/CARD/DIGITAL_WALLET, dynamic TAX_RATE from org config, tips, discounts, receipts, offline mode (2.2) |
| Offline UX         | Sync status (pending/failed counts), retry failed, auto-cleanup (7 days), vite-plugin-pwa for admin app shell caching |
| Print Support      | Browser print CSS for receipts and reports (gap #12)                     |
| Commission Engine  | 3 models, daily auto-calculation, tips distribution, per-tier template defaults (2.4) |
| Payroll            | Summary, export, manager approval workflow (2.4.4–2.4.5)                 |
| Inventory          | Product catalog, stock-in/out/adjust UI, low-stock alerts, COGS (2.5)    |
| Xendit Integration | Payment adapter interface, webhook endpoint, charge creation, saved payment methods (2.2.3, 2.2.5a) ✅ |


---

### Phase 5: Loyalty & Engagement ✅ Complete
...
### Phase 6: Super Admin & Analytics ✅ Complete

> HQ-level oversight and intelligence (integrated into admin app, not a separate app)
...
| Analytics           | Branch comparison, peak heatmap, retention analysis, forecasting, per-barber utilization (3.2) |
| Reports             | Branch reporting, barber leaderboard, exportable CSV/PDF, scheduled email reports, saved templates (2.7) |
...
### Phase 7: SaaS Platform Refactor ✅ Complete
...
- [database_schema.md](database_schema.md) — Complete target Prisma schema (~56 models)

---

## Sprint 10: Multi-Currency Support + AI/ML Features (Complete)

### 10A: Multi-Currency Support
- Dynamic org currency propagated through `GET /auth/me` session (`organization.currency`, `currencySymbol`, `locale`)
- Loyalty config-driven rates via `POINTS_EARN_RATE` / `POINTS_REDEEM_RATE` from PlatformConfig
- Xendit adapter `currency` parameter for non-IDR organizations
- PDF/CSV reports: `Intl.NumberFormat` currency formatting with org config
- Shared `formatCurrency()` utility in admin + client `lib/utils.ts`
- `OrgCurrency` type in both session stores; ~20 frontend files updated

### 10B: Demand Forecasting + Smart Scheduling
- `DemandForecast` model: per-branch, per-day predictions with confidence bands
- Time-series decomposition: 7-day MA trend, day-of-week seasonal indices, linear regression projection
- Holiday dampening (0.3×) via `BranchHoliday` lookup; `SurgeRule` awareness
- MAPE accuracy metric computed from last 7 days of backtested data
- Nightly cron at 02:15 UTC for all active branches
- `ScheduleSuggestion` model with PENDING/ACCEPTED/REJECTED workflow
- Demand-capacity matching: `predictedTransactions × avgServiceDuration` vs scheduled shift hours
- Accept auto-creates `ShiftSchedule` entry
- Admin: 2 new tabs (Forecast, Smart Schedule) on Analytics page

### 10C: Churn Prediction
- `ChurnScore` model with 4 risk tiers (LOW/MEDIUM/HIGH/CRITICAL)
- Weighted RFM scoring: recency (0.35), frequency (0.30), monetary (0.20), engagement (0.15)
- Engagement signals: loyalty account, reviews, referrals, points activity
- Weekly batch cron Monday 04:00 UTC
- Admin: Churn Risk tab with risk filtering and customer detail table

### New Dependencies
- API: `simple-statistics` (statistical computations for forecasting)

### New Prisma Models (3)
- `DemandForecast`, `ScheduleSuggestion`, `ChurnScore`

### New Enums (2)
- `SuggestionStatus`, `ChurnRiskLevel`
