...
#   - Database seeded (pnpm --filter @tmng/saas-api db:seed)
#
# =============================================================================
...
run_api_tests() {
  ...
  if ! check_server "http://localhost:8787/api/health" "API Server"; then
    ...
    echo -e "${YELLOW}Start with: pnpm --filter @tmng/saas-api dev${NC}"
    ((SKIP_COUNT++))
    return
  fi
...
run_client_playwright() {
  ...
  if ! check_server "http://localhost:5174" "Client Dev Server"; then
    ...
    echo -e "${YELLOW}Start with: pnpm --filter @tmng/barber-client dev${NC}"
    ((SKIP_COUNT++))
    return
  fi
...
run_admin_playwright() {
  ...
  if ! check_server "http://localhost:5175" "Admin Dev Server"; then
    ...
    echo -e "${YELLOW}Start with: pnpm --filter @tmng/barber-admin dev${NC}"
    ((SKIP_COUNT++))
    return
  fi
