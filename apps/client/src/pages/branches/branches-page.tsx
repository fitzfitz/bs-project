...
export default function BranchDiscoveryPage() {
  ...
  const centerPos = [-6.2, 106.816666] as LatLngExpression;

  const isMap = viewMode === "map";

  return (
    <div
      className={`flex flex-col bg-slate-50 ${isMap ? "h-dvh overflow-hidden" : "min-h-full pb-20"}`}
    >
      {/* Header & Search */}
                  ...
                  <div className="mt-3 space-y-3 text-sm text-slate-600 border-t border-slate-100 pt-3">
                    ...
                    {branch.operatingHours.length > 0 && (
                      <div className="flex flex-col gap-2">
                        ...
                        <div className="ml-5 grid grid-cols-1 gap-1.5 min-w-[200px]">
                          {[
                            "MONDAY",
                            "TUESDAY",
                            "WEDNESDAY",
                            ...
                            const h = branch.operatingHours.find(
                              (hour) => hour.dayOfWeek === day,
                            );
                            if (!h) return null;
                            const tKey = day.toLowerCase();
                            return (
                              <div
                                key={day}
                ...
                (branch) =>
                  ...
                  branch.longitude && (
                    <Marker
                      key={branch.id}
                      position={
                        [branch.latitude, branch.longitude] as LatLngExpression
                      }
                    >
                      <Popup>
